...
We repeat from step 2 (compute the cost function) to step 4 (update the parameters) for multiple iterations, until we reach a point (a local or global minimum) where the cost function doesn't decrease further. In other words, parameters $w$ and $b$ don't change much with each iteration, and the cost function doesn't decrease significantly. This is called [**convergence**](https://developers.google.com/machine-learning/glossary#convergence).
...
Many Iteration of {
    ...
    2. Backward Pass (Backpropagation): Compute the gradient and update the parameters to reduce the cost.
}
```
